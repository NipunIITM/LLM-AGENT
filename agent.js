...
class LLM_AGENT {
    ...
    async init() {
        try {
            ...
            console.log(`LLM_AGENT v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize LLM_AGENT:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'LLM_AGENT', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to LLM_AGENT! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new LLM_AGENT();
});
